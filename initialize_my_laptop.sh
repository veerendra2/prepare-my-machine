...
# Description: A fancy script to installs necessary packages in Ubuntu
...
install() {
  ...
  if [ "$(cat exitcode)" != "0" ]; then
    printf "${RED}\b[FAILED]${PLAIN}"
  else
    printf "${GREEN}\b[DONE]${PLAIN}"
  fi
}
...
install_pkgs(){
  ...
  echo $LINE
  install "sudo apt-get update && sudo apt-get upgrade -y". "[*] Updating"
